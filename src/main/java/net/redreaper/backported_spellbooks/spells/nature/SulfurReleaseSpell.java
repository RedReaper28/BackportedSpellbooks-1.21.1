...
public class SulfurReleaseSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(BackportedSpellbooks.MOD_ID, "sulfur_release");
    ...
    public Optional<SoundEvent> getCastFinishSound() {
        return Optional.of(ModSoundEvents.SULFUR_CUBE_DEATH.get());
    }
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        MagicManager.spawnParticles(level, ModParticles.NOXIOUS_GAS.get(), entity.getX(), entity.getY() + (double) 1.0F, entity.getZ(), 250, (double) 0.0F, (double) 0.0F, (double) 0.0F, (double) 1.0F, false);
        level.getEntities(entity, entity.getBoundingBox().inflate((double) radius, (double) 4.0F, (double) radius), (target) -> !DamageSources.isFriendlyFireBetween(target, entity) && Utils.hasLineOfSight(level, entity, target, false)).forEach((target) -> {
            if (target instanceof LivingEntity livingEntity) {
                if (livingEntity.distanceToSqr(entity) < (double) (radius * radius)) {
                    ...
                    livingEntity.addEffect(new MobEffectInstance(ModMobEffects.SULFURIC_POISON, i, this.getWitherAmplifier(spellLevel, entity)));
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.CONFUSION, i, this.getWitherAmplifier(spellLevel, entity)));
                }
            }
        });
